...
export const MomCommand = async (interaction: CommandInteraction): Promise<void> => {
    try {
        ...
        const redditRresponse = await GetRedditPosts("momsgonewild", 5)
        ...
        if(/(http|https)\:\/\/(www\.)?redgifs\.com\/watch\/[a-zA-Z]{3,35}(\/)?/g.test(entity.image)){
            ...
            row.addComponents(
                new ButtonBuilder()
                    .setCustomId(`mom-${redditRresponse.after}-${interaction.user.id}`)
                    .setLabel("Next")
...
export const NextMomButton = async (interaction: ButtonInteraction, args: Array<string>): Promise<void> => {
    try {
        ...
        const redditRresponse = await GetRedditPosts("momsgonewild", 2, args[0])
