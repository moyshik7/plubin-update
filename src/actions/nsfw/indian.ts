...
export const IndianCommand = async (interaction: CommandInteraction): Promise<void> => {
    try {
        ...
        const redditRresponse = await GetRedditPosts("indiansgonewild", 5)
        ...
        if(/(http|https)\:\/\/(www\.)?redgifs\.com\/watch\/[a-zA-Z]{3,35}(\/)?/g.test(entity.image)){
            ...
            row.addComponents(
                new MessageButton()
                    .setCustomId(`indian-${redditRresponse.after}-${interaction.user.id}`)
                    .setLabel("Next")
                    .setStyle("SUCCESS")
            )
            interaction.editReply({
...
export const NextIndianButton = async (interaction: ButtonInteraction, args: Array<string>): Promise<void> => {
    try {
        ...
        const redditRresponse = await GetRedditPosts("indiansgonewild", 2, args[0])
