...
export const OilCommand = async (interaction: CommandInteraction): Promise<void> => {
    try {
        ...
        const redditRresponse = await GetRedditPosts("oilporn", 5)
        ...
        if(/(http|https)\:\/\/(www\.)?redgifs\.com\/watch\/[a-zA-Z]{3,35}(\/)?/g.test(entity.image)){
            ...
            row.addComponents(
                new ButtonBuilder()
                    .setCustomId(`oil-${redditRresponse.after}-${interaction.user.id}`)
                    .setLabel("Next")
...
export const NextOilButton = async (interaction: ButtonInteraction, args: Array<string>): Promise<void> => {
    try {
        ...
        const redditRresponse = await GetRedditPosts("oilporn", 2, args[0])
