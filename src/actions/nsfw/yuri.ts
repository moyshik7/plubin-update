...
export const YuriCommand = async (interaction: CommandInteraction): Promise<void> => {
    try {
        ...
        row.addComponents(
            new MessageButton()
                .setCustomId(`yuri-${redditRresponse.after}-${interaction.user.id}`)
                .setLabel("Next")
                .setStyle("SUCCESS")
        )
...
export const NextYuriButton = async (interaction: ButtonInteraction, args: Array<string>): Promise<void> => {
    try {
