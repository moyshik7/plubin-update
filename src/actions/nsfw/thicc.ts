...
export const ThiccCommand = async (interaction: CommandInteraction): Promise<void> => {
    try {
        ...
        const redditRresponse = await GetRedditPosts("thick", 5)
        ...
        if(/(http|https)\:\/\/(www\.)?redgifs\.com\/watch\/[a-zA-Z]{3,35}(\/)?/g.test(entity.image)){
            ...
            row.addComponents(
                new MessageButton()
                    .setCustomId(`thicc-${redditRresponse.after}-${interaction.user.id}`)
                    .setLabel("Next")
                    .setStyle("SUCCESS")
            )
            interaction.editReply({
...
export const NextThiccButton = async (interaction: ButtonInteraction, args: Array<string>): Promise<void> => {
    try {
        ...
        const redditRresponse = await GetRedditPosts("thick", 2, args[0])
