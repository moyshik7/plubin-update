...
export const FilipinoCommand = async (interaction: CommandInteraction): Promise<void> => {
    try {
        ...
        const redditRresponse = await GetRedditPosts("filipinohotties", 5)
        ...
        if(/(http|https)\:\/\/(www\.)?redgifs\.com\/watch\/[a-zA-Z]{3,35}(\/)?/g.test(entity.image)){
            ...
            row.addComponents(
                new ButtonBuilder()
                    .setCustomId(`filipino-${redditRresponse.after}-${interaction.user.id}`)
                    .setLabel("Next")
...
export const NextFilipinoButton = async (interaction: ButtonInteraction, args: Array<string>): Promise<void> => {
    try {
        ...
        const redditRresponse = await GetRedditPosts("filipinohotties", 2, args[0])
