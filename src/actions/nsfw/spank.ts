...
export const SpankCommand = async (interaction: CommandInteraction): Promise<void> => {
    try {
        ...
        const redditRresponse = await GetRedditPosts("spanking", 5)
        ...
        if(/(http|https)\:\/\/(www\.)?redgifs\.com\/watch\/[a-zA-Z]{3,35}(\/)?/g.test(entity.image)){
            ...
            row.addComponents(
                new ButtonBuilder()
                    .setCustomId(`spank-${redditRresponse.after}-${interaction.user.id}`)
                    .setLabel("Next")
...
export const NextSpankButton = async (interaction: ButtonInteraction, args: Array<string>): Promise<void> => {
    try {
        ...
        const redditRresponse = await GetRedditPosts("spanking", 2, args[0])
