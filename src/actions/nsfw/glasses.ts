...
export const GlassesCommand = async (interaction: CommandInteraction): Promise<void> => {
    try {
        ...
        if(/(http|https)\:\/\/(www\.)?redgifs\.com\/watch\/[a-zA-Z]{3,35}(\/)?/g.test(entity.image)){
            ...
            row.addComponents(
                new ButtonBuilder()
                    .setCustomId(`glasses-${redditRresponse.after}-${interaction.user.id}`)
                    .setLabel("Next")
...
export const NextGlassesButton = async (interaction: ButtonInteraction, args: Array<string>): Promise<void> => {
    try {
