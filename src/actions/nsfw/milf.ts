...
export const MilfCommand = async (interaction: CommandInteraction): Promise<void> => {
    try {
        ...
        const redditRresponse = await GetRedditPosts("milf", 5)
        ...
        if(/(http|https)\:\/\/(www\.)?redgifs\.com\/watch\/[a-zA-Z]{3,35}(\/)?/g.test(entity.image)){
            ...
            row.addComponents(
                new ButtonBuilder()
                    .setCustomId(`milf-${redditRresponse.after}-${interaction.user.id}`)
                    .setLabel("Next")
...
export const NextMilfButton = async (interaction: ButtonInteraction, args: Array<string>): Promise<void> => {
    try {
        ...
        const redditRresponse = await GetRedditPosts("milf", 2, args[0])
